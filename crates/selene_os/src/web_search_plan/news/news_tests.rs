...
use crate::web_search_plan::news::recency::normalize_published_at;
use crate::web_search_plan::news::{
    append_news_audit_fields, execute_news_ladder_from_tool_request, NewsRuntimeConfig,
};
use crate::web_search_plan::proxy::proxy_config::ProxyConfig;
use crate::web_search_plan::proxy::ProxyMode;
use crate::web_search_plan::web_provider::health_state::HealthPolicy;
...
fn test_t2_brave_empty_assist_executed() {
    ...
    .expect("assist path should pass");
...
fn test_t6_diversity_threshold_deterministic() {
    ...
    let result_b = execute_news_ladder_from_tool_request(
        &tool_request_packet("supply chain", "high"),
        1_772_496_000_000i64,
        &config,
    )
...
fn test_t8_evidence_packet_completeness_news_mode() {
    ...
    append_news_audit_fields(&mut audit_packet, &result.audit_metrics)
        .expect("audit append should pass");
    assert!(audit_packet
        .pointer("/turn_state_transition/news_audit/lead_attempted")
        .is_some());
    assert!(audit_packet
        .pointer("/turn_state_transition/news_audit/fallback_used")
        .is_some());
    assert!(audit_packet
        .pointer("/turn_state_transition/news_audit/fallback_reason_code")
        .is_some());
    assert!(audit_packet
        .pointer("/turn_state_transition/news_audit/recency_window_applied")
        .is_some());
    assert!(audit_packet
        .pointer("/turn_state_transition/news_audit/filtered_by_recency_count")
        .is_some());
    assert!(audit_packet
        .pointer("/turn_state_transition/news_audit/distinct_domain_count")
        .is_some());
    assert!(audit_packet
        .pointer("/turn_state_transition/news_audit/diversity_threshold_met")
        .is_some());
    assert!(audit_packet
        .pointer("/turn_state_transition/news_audit/contradiction_clusters_detected")
        .is_some());
    assert!(audit_packet
        .pointer("/turn_state_transition/news_audit/assist_used")
        .is_some());
