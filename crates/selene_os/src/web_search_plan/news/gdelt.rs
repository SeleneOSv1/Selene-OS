...
pub fn execute_gdelt_news_search(
    endpoint: &str,
    query: &str,
    max_results: usize,
    timeout_ms: u64,
    ...
    if let Err(check) = run_startup_self_check(proxy_config) {
            ...
            && check.severity.as_str() == "critical"
        {
            return Err(NewsProviderError {
                provider_id: NewsProviderId::GdeltNewsAssist,
    ...
    let response = agent
        ...
        .query("mode", "ArtList")
        .query("sort", "DateDesc")
        .query("maxrecords", &max_results.to_string())
        .query("format", "json")
        .call()
    ...
    let body: Value =
        serde_json::from_reader(response.into_reader()).map_err(|_| NewsProviderError {
            ...
            status_code: None,
            message: "gdelt JSON parse failed".to_string(),
            latency_ms: start.elapsed().as_millis() as u64,
        })?;
    ...
    for article in articles {
        ...
        let snippet = article
            .get("snippet")
            .or_else(|| article.get("seendate"))
            .and_then(Value::as_str)
            .map(normalize_text_value)
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| "No snippet available.".to_string());

        let published_raw = article
            .get("seendate")
            .or_else(|| article.get("published"))
            .and_then(Value::as_str)
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .map(ToString::to_string);

        let trust_tier = article
            .get("sourcecountry")
            .or_else(|| article.get("tone"))
            .and_then(Value::as_str)
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .map(ToString::to_string);
...
fn map_ureq_error(err: ureq::Error, latency_ms: u64) -> NewsProviderError {
    match err {
        ureq::Error::Status(status, _) => NewsProviderError {
            ...
            message: format!("gdelt HTTP status {}", status),
            latency_ms,
        },
        ureq::Error::Transport(transport) => {
            ...
            NewsProviderError {
                ...
                kind,
                status_code: None,
                message: "gdelt transport failure".to_string(),
                latency_ms,
            }
        }
    }
