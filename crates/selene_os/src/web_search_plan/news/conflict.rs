...
use crate::web_search_plan::news::NormalizedNewsResult;
...
pub fn build_conflict_clusters(results: &[NormalizedNewsResult]) -> Vec<ConflictCluster> {
    ...
    for (topic_key, group) in grouped {
        ...
        let mut conflicting_claims = BTreeSet::new();

        for result in group {
            let polarity = polarity(result);
            has_positive |= polarity >= 0;
            has_negative |= polarity <= 0;
            source_refs.insert(result.url.clone());
            conflicting_claims.insert(result.snippet.clone());
        }
        ...
        out.push(ConflictCluster {
            ...
            conflicting_claims: conflicting_claims.into_iter().collect(),
        });
