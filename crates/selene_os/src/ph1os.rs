...
mod tests {
    ...
    #[test]
    fn at_os_22j_voice_live_entrypoint_rejects_requested_trigger_mismatch() {
        let actor_user_id = UserId::new("tenant_1:os_live_voice_user").unwrap();
        let device_id = DeviceId::new("os_live_voice_device_11").unwrap();
        let voice_context =
            voice_context_ios_explicit().expect("ios explicit voice context must exist");
        let mut runtime_execution_envelope =
            runtime_envelope_for_voice_context(&actor_user_id, &device_id, voice_context);
        runtime_execution_envelope.platform_context =
            ...
            .expect("ios wake platform context must build");
        runtime_execution_envelope
            .validate()
            .expect("trigger-mismatch envelope must remain contract-valid");

        assert_eq!(voice_context.platform, OsVoicePlatform::Ios);
        assert_eq!(voice_context.trigger, OsVoiceTrigger::Explicit);
        assert_eq!(runtime_execution_envelope.platform, AppPlatform::Ios);
        assert_eq!(
            runtime_execution_envelope.platform_context.platform_type,
            AppPlatform::Ios
        );
        ...
        .expect_err("requested-trigger mismatch must be rejected by PH1.OS voice governance");
        match input {
            ContractViolation::InvalidValue { field, reason } => {
                assert_eq!(
                    field,
                    "os_voice_live_turn_input.runtime_execution_envelope.platform_context.requested_trigger"
                );
                assert_ne!(
                    field,
                    "os_voice_live_turn_input.runtime_execution_envelope.platform"
                ...
                assert_eq!(
                    reason,
                    "must match os_voice_live_turn_input.top_level_turn_input.voice_context.trigger"
                );
            }
            _ => panic!("expected invalid-value contract violation"),
        }
    ...
    #[test]
    fn at_os_22l_voice_live_entrypoint_rejects_ios_explicit_unsupported_wake_word_capability() {
        let actor_user_id = UserId::new("tenant_1:os_live_voice_user").unwrap();
        ...
        runtime_execution_envelope
            .platform_context
            .negotiated_capabilities
            .push(DeviceCapability::WakeWord);
        ...
        .expect_err("ios explicit unsupported wake-word capability must fail closed");
        match input {
            ContractViolation::InvalidValue { field, reason } => {
                assert_eq!(field, "platform_runtime_context.negotiated_capabilities");
                assert_eq!(reason, "contains capability unsupported by platform");
            }
            _ => panic!("expected invalid-value contract violation"),
        }
    }
    ...
    #[test]
    fn at_os_22m_voice_live_entrypoint_rejects_ios_explicit_claimed_unsupported_wake_word_capability(
    ) {
        let actor_user_id = UserId::new("tenant_1:os_live_voice_user").unwrap();
        let device_id = DeviceId::new("os_live_voice_device_12").unwrap();
        let voice_context =
        ...
        .expect_err("ios explicit claimed unsupported wake-word capability must fail closed");
        match input {
            ContractViolation::InvalidValue { field, reason } => {
                assert_eq!(field, "platform_runtime_context.claimed_capabilities");
                assert_eq!(reason, "contains capability unsupported by platform");
            }
            _ => panic!("expected invalid-value contract violation"),
        }
    ...
    fn at_os_22n_voice_live_entrypoint_rejects_ios_explicit_claimed_camera_capability_missing_from_negotiated_capabilities() {
        ...
        .expect_err("ios explicit claimed camera capability missing from negotiated capabilities must fail closed");
        match input {
            ContractViolation::InvalidValue { field, reason } => {
                assert_eq!(field, "platform_runtime_context.claimed_capabilities");
                assert_eq!(
                    reason,
                    "claimed capabilities must be present in negotiated capabilities"
                );
            }
            _ => panic!("expected invalid-value contract violation"),
        }
