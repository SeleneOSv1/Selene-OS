...
use selene_storage::repo::{Ph1NlpRepo, Ph1fFoundationRepo, Ph1jAuditRepo};
...
#[test]
fn at_nlp_db_01_tenant_isolation_enforced() {
    ...
    s.ph1nlp_intent_draft_commit_row(
        MonotonicTimeNs(100),
        "tenant_a".to_string(),
        CorrelationId(1001),
        TurnId(1),
        ...
        "HIGH".to_string(),
        ReasonCodeId(0x4E00_1001),
        "nlp-tenant-a".to_string(),
    )
    .unwrap();

    s.ph1nlp_clarify_commit_row(
        MonotonicTimeNs(101),
        "tenant_b".to_string(),
        CorrelationId(2001),
        TurnId(1),
        ...
        device_b.clone(),
        "when".to_string(),
        ReasonCodeId(0x4E00_1002),
        "nlp-tenant-b".to_string(),
    )
    .unwrap();

    let tenant_mismatch = s.ph1nlp_chat_commit_row(
        MonotonicTimeNs(102),
        "tenant_b".to_string(),
        CorrelationId(3001),
        TurnId(1),
        ...
        user_a,
        device_a,
        ReasonCodeId(0x4E00_1003),
        "nlp-tenant-mismatch".to_string(),
    );
    assert!(matches!(
        tenant_mismatch,
...
#[test]
fn at_nlp_db_02_append_only_enforced() {
    let mut s = Ph1fStore::new_in_memory();
    ...
    let event_id = s
        .ph1nlp_intent_draft_commit_row(
            ...
            u,
            d,
            "TIME_QUERY".to_string(),
            "HIGH".to_string(),
            ReasonCodeId(0x4E00_2001),
            "nlp-append".to_string(),
        )
        .unwrap();
...
#[test]
fn at_nlp_db_03_idempotency_dedupe_works() {
    let mut s = Ph1fStore::new_in_memory();
    ...
    let second = s
        .ph1nlp_clarify_commit_row(
            ...
            d,
            "amount".to_string(),
            ReasonCodeId(0x4E00_3002),
            "nlp-idem".to_string(),
        )
        .unwrap();

    assert_eq!(first, second);
    assert_eq!(s.ph1nlp_audit_rows(corr).len(), 1);
}

#[test]
fn at_nlp_db_04_no_current_table_rebuild_required() {
    let mut s = Ph1fStore::new_in_memory();
    ...
    s.ph1nlp_intent_draft_commit_row(
        ...
        "MED".to_string(),
        ReasonCodeId(0x4E00_4001),
        "nlp-current-intent".to_string(),
    )
    .unwrap();
    ...
    s.ph1nlp_clarify_commit_row(
        ...
        d.clone(),
        "when".to_string(),
        ReasonCodeId(0x4E00_4002),
        "nlp-current-clarify".to_string(),
    )
    .unwrap();

    s.ph1nlp_chat_commit_row(
        MonotonicTimeNs(402),
        "tenant_a".to_string(),
        corr,
        TurnId(3),
        ...
        u,
        d,
        ReasonCodeId(0x4E00_4003),
        "nlp-current-chat".to_string(),
    )
    .unwrap();

    // Row 14 is ledger-only on `audit_events`; no PH1.NLP-owned current table exists.
    assert_eq!(s.ph1nlp_audit_rows(corr).len(), 3);
}
