...
use selene_kernel_contracts::ph1bcast::{
    ...
    Ph1BcastRequest, Ph1BcastResponse, BCAST_DEFAULT_MAX_FOLLOWUP_ATTEMPTS,
    BCAST_DEFAULT_NON_URGENT_WAIT_SECONDS, BCAST_DEFAULT_URGENT_FOLLOWUP_IMMEDIATE,
...
const REMINDER_DELIVERY_RETRY_DELAY_NS: u64 = 300_000_000_000; // 5 minutes
const REMINDER_DELIVERY_MISSED_GRACE_NS: u64 = 60_000_000_000; // 1 minute
const REMINDER_DELIVERY_FOLLOWUP_DELAY_NS: u64 = 900_000_000_000; // 15 minutes
...
mod reminder_reason_codes {
    ...
    pub const REM_FAIL_POLICY_BLOCKED: ReasonCodeId = ReasonCodeId(0x5245_00F5);
}
...
pub struct Ph1fStore {
    ...
    bcast_policy_ledger: Vec<BcastPolicyLedgerRow>,
    bcast_policy_current: BTreeMap<TenantId, BcastPolicyCurrentRecord>,
    // Idempotency: (tenant_id, idempotency_key) -> bcast_policy_event_id.
    bcast_policy_idempotency_index: BTreeMap<(TenantId, String), u64>,
    // Primary-key uniqueness: bcast_policy_event_id -> event_idx.
    bcast_policy_event_lookup: BTreeMap<u64, usize>,
    ...
    bcast_urgent_followup_policy_event_lookup: BTreeMap<u64, usize>,
    ...
    next_bcast_policy_event_id: u64,
    next_bcast_wait_policy_event_id: u64,
    next_bcast_urgent_followup_policy_event_id: u64,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum BcastPolicySettingKey {
    NonUrgentWaitSeconds,
    UrgentFollowupMode,
    MaxFollowupAttempts,
    ReminderDefaultTime,
}

impl BcastPolicySettingKey {
    pub fn as_str(self) -> &'static str {
        match self {
            BcastPolicySettingKey::NonUrgentWaitSeconds => "non_urgent_wait_seconds",
            BcastPolicySettingKey::UrgentFollowupMode => "urgent_followup_mode",
            BcastPolicySettingKey::MaxFollowupAttempts => "max_followup_attempts",
            BcastPolicySettingKey::ReminderDefaultTime => "reminder_default_time",
        }
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum BcastPolicyUpdateValue {
    NonUrgentWaitSeconds(u32),
    UrgentFollowupMode { immediate: bool },
    MaxFollowupAttempts(u32),
    ReminderDefaultTime(Option<String>),
}

impl BcastPolicyUpdateValue {
    pub fn setting_key(&self) -> BcastPolicySettingKey {
        match self {
            BcastPolicyUpdateValue::NonUrgentWaitSeconds(_) => {
                BcastPolicySettingKey::NonUrgentWaitSeconds
            }
            BcastPolicyUpdateValue::UrgentFollowupMode { .. } => {
                BcastPolicySettingKey::UrgentFollowupMode
            }
            BcastPolicyUpdateValue::MaxFollowupAttempts(_) => {
                BcastPolicySettingKey::MaxFollowupAttempts
            }
            BcastPolicyUpdateValue::ReminderDefaultTime(_) => {
                BcastPolicySettingKey::ReminderDefaultTime
            }
        }
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BcastPolicyLedgerRow {
    pub schema_version: SchemaVersion,
    pub bcast_policy_event_id: u64,
    pub tenant_id: TenantId,
    pub setting_key: BcastPolicySettingKey,
    pub old_value: String,
    pub new_value: String,
    pub non_urgent_wait_seconds: u32,
    pub urgent_followup_immediate: bool,
    pub max_followup_attempts: u32,
    pub reminder_default_time: Option<String>,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BcastPolicyCurrentRecord {
    pub schema_version: SchemaVersion,
    pub tenant_id: TenantId,
    pub non_urgent_wait_seconds: u32,
    pub urgent_followup_immediate: bool,
    ...
    pub reason_code: ReasonCodeId,
    pub policy_version: u64,
    pub last_bcast_policy_event_id: u64,
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BcastUrgentFollowupPolicyLedgerRow {
    pub schema_version: SchemaVersion,
    pub bcast_urgent_followup_policy_event_id: u64,
    pub tenant_id: TenantId,
    pub urgent_followup_immediate: bool,
    pub updated_by_user_id: UserId,
    pub correlation_id: CorrelationId,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BcastUrgentFollowupPolicyCurrentRecord {
    pub schema_version: SchemaVersion,
    pub tenant_id: TenantId,
    pub urgent_followup_immediate: bool,
    pub updated_by_user_id: UserId,
    ...
    pub reason_code: ReasonCodeId,
    pub policy_version: u64,
    pub last_bcast_urgent_followup_policy_event_id: u64,
}
...
impl Ph1fStore {
    pub fn new_in_memory() -> Self {
        Self {
            ...
            bcast_policy_ledger: Vec::new(),
            bcast_policy_current: BTreeMap::new(),
            bcast_policy_idempotency_index: BTreeMap::new(),
            bcast_policy_event_lookup: BTreeMap::new(),
            ...
            bcast_urgent_followup_policy_ledger: Vec::new(),
            bcast_urgent_followup_policy_current: BTreeMap::new(),
            bcast_urgent_followup_policy_idempotency_index: BTreeMap::new(),
            bcast_urgent_followup_policy_event_lookup: BTreeMap::new(),
            ...
            next_bcast_policy_event_id: 1,
            next_bcast_wait_policy_event_id: 1,
            next_bcast_urgent_followup_policy_event_id: 1,
            next_reminder_seq: 1,
    ...
    fn bcast_policy_defaults() -> (u32, bool, u32, Option<String>) {
        (
            BCAST_DEFAULT_NON_URGENT_WAIT_SECONDS,
            BCAST_DEFAULT_URGENT_FOLLOWUP_IMMEDIATE,
            BCAST_DEFAULT_MAX_FOLLOWUP_ATTEMPTS,
            None,
        )
    }

    fn bcast_policy_values_for_tenant(
        &self,
        tenant_id: &TenantId,
    ) -> (u32, bool, u32, Option<String>) {
        if let Some(current) = self.bcast_policy_current.get(tenant_id) {
            return (
                current.non_urgent_wait_seconds,
                current.urgent_followup_immediate,
                current.max_followup_attempts,
                current.reminder_default_time.clone(),
            );
        }
        Self::bcast_policy_defaults()
    }

    fn format_bcast_policy_value(
        setting_key: BcastPolicySettingKey,
        non_urgent_wait_seconds: u32,
        urgent_followup_immediate: bool,
        max_followup_attempts: u32,
        reminder_default_time: Option<&str>,
    ) -> String {
        match setting_key {
            BcastPolicySettingKey::NonUrgentWaitSeconds => non_urgent_wait_seconds.to_string(),
            BcastPolicySettingKey::UrgentFollowupMode => {
                if urgent_followup_immediate {
                    "immediate".to_string()
                } else {
                    "wait".to_string()
                }
            }
            BcastPolicySettingKey::MaxFollowupAttempts => max_followup_attempts.to_string(),
            BcastPolicySettingKey::ReminderDefaultTime => reminder_default_time
    ...
    fn apply_bcast_policy_event_to_current(&mut self, ev: &BcastPolicyLedgerRow) {
        self.bcast_policy_current.insert(
            ev.tenant_id.clone(),
            BcastPolicyCurrentRecord {
                schema_version: PH1BCAST_CONTRACT_VERSION,
                tenant_id: ev.tenant_id.clone(),
                non_urgent_wait_seconds: ev.non_urgent_wait_seconds,
                urgent_followup_immediate: ev.urgent_followup_immediate,
                max_followup_attempts: ev.max_followup_attempts,
                reminder_default_time: ev.reminder_default_time.clone(),
                ...
                last_bcast_policy_event_id: ev.bcast_policy_event_id,
            },
        );
    }

    #[allow(clippy::too_many_arguments)]
    pub fn append_bcast_policy_update_event(
        &mut self,
        tenant_id: TenantId,
        update: BcastPolicyUpdateValue,
        ...
        validate_comms_idempotency_key("bcast_policy_ledger.idempotency_key", &idempotency_key)?;
        let idx = (tenant_id.clone(), idempotency_key.clone());
        if let Some(existing_event_id) = self.bcast_policy_idempotency_index.get(&idx) {
            return Ok(*existing_event_id);
        }

        let (
            mut non_urgent_wait_seconds,
            mut urgent_followup_immediate,
            mut max_followup_attempts,
            mut reminder_default_time,
        ) = self.bcast_policy_values_for_tenant(&tenant_id);
        let setting_key = update.setting_key();
        let old_value = Self::format_bcast_policy_value(
            setting_key,
            non_urgent_wait_seconds,
            urgent_followup_immediate,
            max_followup_attempts,
            reminder_default_time.as_deref(),
        );

        match update {
            BcastPolicyUpdateValue::NonUrgentWaitSeconds(wait_seconds) => {
                if wait_seconds == 0 {
                    return Err(StorageError::ContractViolation(
                        ContractViolation::InvalidValue {
                            field: "bcast_policy_ledger.non_urgent_wait_seconds",
                            reason: "must be >= 1",
                        },
                    ));
                }
                non_urgent_wait_seconds = wait_seconds;
            }
            BcastPolicyUpdateValue::UrgentFollowupMode { immediate } => {
                urgent_followup_immediate = immediate;
            }
            BcastPolicyUpdateValue::MaxFollowupAttempts(attempts) => {
                if attempts == 0 {
                    return Err(StorageError::ContractViolation(
                        ContractViolation::InvalidValue {
                            field: "bcast_policy_ledger.max_followup_attempts",
                            reason: "must be >= 1",
                        },
                    ));
                }
            ...
            BcastPolicyUpdateValue::ReminderDefaultTime(value) => {
                if let Some(v) = value.as_ref() {
                    if v.trim().is_empty() {
                        return Err(StorageError::ContractViolation(
                            ContractViolation::InvalidValue {
                                field: "bcast_policy_ledger.reminder_default_time",
                                reason: "must not be empty when provided",
                            },
                        ));
                    }
                ...
                reminder_default_time = value;
            }
        }

        let new_value = Self::format_bcast_policy_value(
            setting_key,
            non_urgent_wait_seconds,
            urgent_followup_immediate,
            max_followup_attempts,
        ...
        let event_id = self.next_bcast_policy_event_id;
        self.next_bcast_policy_event_id = self.next_bcast_policy_event_id.saturating_add(1);
        let policy_version = self
            .bcast_policy_current
            .get(&tenant_id)
            .map(|v| v.policy_version.saturating_add(1))
            .unwrap_or(1);

        let row = BcastPolicyLedgerRow {
            schema_version: PH1BCAST_CONTRACT_VERSION,
            bcast_policy_event_id: event_id,
        ...
        let event_idx = self.bcast_policy_ledger.len();
        self.bcast_policy_event_lookup.insert(event_id, event_idx);
        self.bcast_policy_idempotency_index.insert(idx, event_id);
        self.bcast_policy_ledger.push(row);
        Ok(event_id)
    }

    pub fn bcast_policy_ledger(&self) -> &[BcastPolicyLedgerRow] {
        &self.bcast_policy_ledger
    }

    pub fn bcast_policy_row(&self, bcast_policy_event_id: u64) -> Option<&BcastPolicyLedgerRow> {
        let idx = self
            .bcast_policy_event_lookup
            .get(&bcast_policy_event_id)
            .copied()?;
        self.bcast_policy_ledger.get(idx)
    }

    pub fn bcast_policy_current(&self) -> &BTreeMap<TenantId, BcastPolicyCurrentRecord> {
        &self.bcast_policy_current
    }

    pub fn bcast_policy_current_row(
        &self,
        tenant_id: &TenantId,
    ) -> Option<&BcastPolicyCurrentRecord> {
        self.bcast_policy_current.get(tenant_id)
    }

    pub fn bcast_policy_changes_for_tenant(
        &self,
        tenant_id: &TenantId,
        limit: usize,
    ) -> Vec<BcastPolicyLedgerRow> {
        if limit == 0 {
            return Vec::new();
        }
        self.bcast_policy_ledger
    ...
    pub fn rebuild_bcast_policy_current_from_ledger(&mut self) -> Result<(), StorageError> {
        self.bcast_policy_current.clear();
        self.bcast_policy_idempotency_index.clear();
        self.bcast_policy_event_lookup.clear();

        for (idx, row) in self.bcast_policy_ledger.iter().enumerate() {
            if self
                .bcast_policy_event_lookup
                .insert(row.bcast_policy_event_id, idx)
                .is_some()
            {
                return Err(StorageError::DuplicateKey {
                    table: "bcast_policy_ledger",
                    key: row.bcast_policy_event_id.to_string(),
                });
            }
        }

        let mut ordered = self.bcast_policy_ledger.clone();
        ...
        for row in ordered {
            ...
            if row.max_followup_attempts == 0 {
                return Err(StorageError::ContractViolation(
                    ContractViolation::InvalidValue {
                        field: "bcast_policy_ledger.max_followup_attempts",
                        reason: "must be >= 1",
            ...
            if let Some(value) = row.reminder_default_time.as_ref() {
            ...
            if row.policy_version != expected_policy_version {
                return Err(StorageError::ContractViolation(
                    ContractViolation::InvalidValue {
                        field: "bcast_policy_ledger.policy_version",
    ...
    pub fn attempt_overwrite_bcast_policy_event(
        &mut self,
        _bcast_policy_event_id: u64,
    ) -> Result<(), StorageError> {
        Err(StorageError::AppendOnlyViolation {
            table: "bcast_policy_ledger",
        })
    ...
    fn apply_bcast_urgent_followup_policy_event_to_current(
        &mut self,
        ev: &BcastUrgentFollowupPolicyLedgerRow,
    ) {
        self.bcast_urgent_followup_policy_current.insert(
            ev.tenant_id.clone(),
            BcastUrgentFollowupPolicyCurrentRecord {
                schema_version: PH1BCAST_CONTRACT_VERSION,
                tenant_id: ev.tenant_id.clone(),
                urgent_followup_immediate: ev.urgent_followup_immediate,
                updated_by_user_id: ev.updated_by_user_id.clone(),
                ...
                reason_code: ev.reason_code,
                policy_version: ev.policy_version,
                last_bcast_urgent_followup_policy_event_id: ev
                    .bcast_urgent_followup_policy_event_id,
            },
        );
    }

    #[allow(clippy::too_many_arguments)]
    pub fn append_bcast_urgent_followup_policy_update_event(
        &mut self,
        tenant_id: TenantId,
        urgent_followup_immediate: bool,
        updated_by_user_id: UserId,
        ...
        validate_comms_idempotency_key(
            "bcast_urgent_followup_policy_ledger.idempotency_key",
            &idempotency_key,
        )?;
        let idx = (tenant_id.clone(), idempotency_key.clone());
        if let Some(existing_event_id) = self
        ...
        let event_id = self.next_bcast_urgent_followup_policy_event_id;
        self.next_bcast_urgent_followup_policy_event_id = self
            .next_bcast_urgent_followup_policy_event_id
            .saturating_add(1);
        let policy_version = self
            .bcast_urgent_followup_policy_current
            .get(&tenant_id)
        ...
        let row = BcastUrgentFollowupPolicyLedgerRow {
            schema_version: PH1BCAST_CONTRACT_VERSION,
            bcast_urgent_followup_policy_event_id: event_id,
            tenant_id: tenant_id.clone(),
            urgent_followup_immediate,
            updated_by_user_id,
            correlation_id,
        ...
        self.apply_bcast_urgent_followup_policy_event_to_current(&row);

        let event_idx = self.bcast_urgent_followup_policy_ledger.len();
        self.bcast_urgent_followup_policy_event_lookup
            .insert(event_id, event_idx);
        self.bcast_urgent_followup_policy_idempotency_index
            .insert(idx, event_id);
        self.bcast_urgent_followup_policy_ledger.push(row);
        Ok(event_id)
    }

    pub fn bcast_urgent_followup_policy_ledger(&self) -> &[BcastUrgentFollowupPolicyLedgerRow] {
        &self.bcast_urgent_followup_policy_ledger
    }

    pub fn bcast_urgent_followup_policy_current(
        &self,
    ) -> &BTreeMap<TenantId, BcastUrgentFollowupPolicyCurrentRecord> {
        &self.bcast_urgent_followup_policy_current
    }

    pub fn bcast_urgent_followup_policy_current_row(
        &self,
        tenant_id: &TenantId,
    ) -> Option<&BcastUrgentFollowupPolicyCurrentRecord> {
        self.bcast_urgent_followup_policy_current.get(tenant_id)
    }

    pub fn bcast_urgent_followup_policy_row(
        &self,
        bcast_urgent_followup_policy_event_id: u64,
    ) -> Option<&BcastUrgentFollowupPolicyLedgerRow> {
        let idx = self
            .bcast_urgent_followup_policy_event_lookup
            .get(&bcast_urgent_followup_policy_event_id)
            .copied()?;
        self.bcast_urgent_followup_policy_ledger.get(idx)
    }

    pub fn bcast_urgent_followup_immediate_for_tenant(&self, tenant_id: &TenantId) -> bool {
        self.bcast_policy_current
            .get(tenant_id)
            .map(|v| v.urgent_followup_immediate)
            .or_else(|| {
                self.bcast_urgent_followup_policy_current
                    .get(tenant_id)
                    .map(|v| v.urgent_followup_immediate)
            })
            .unwrap_or(BCAST_DEFAULT_URGENT_FOLLOWUP_IMMEDIATE)
    }

    pub fn bcast_max_followup_attempts_for_tenant(&self, tenant_id: &TenantId) -> u32 {
        self.bcast_policy_current
            .get(tenant_id)
            .map(|v| v.max_followup_attempts)
            .unwrap_or(BCAST_DEFAULT_MAX_FOLLOWUP_ATTEMPTS)
    }

    pub fn bcast_reminder_default_time_for_tenant(&self, tenant_id: &TenantId) -> Option<&str> {
        self.bcast_policy_current
            .get(tenant_id)
            .and_then(|v| v.reminder_default_time.as_deref())
    }

    pub fn rebuild_bcast_urgent_followup_policy_current_from_ledger(
        &mut self,
    ) -> Result<(), StorageError> {
        self.bcast_urgent_followup_policy_current.clear();
        self.bcast_urgent_followup_policy_idempotency_index.clear();
        self.bcast_urgent_followup_policy_event_lookup.clear();

        for (idx, row) in self.bcast_urgent_followup_policy_ledger.iter().enumerate() {
            if self
                .bcast_urgent_followup_policy_event_lookup
                .insert(row.bcast_urgent_followup_policy_event_id, idx)
                .is_some()
            {
                return Err(StorageError::DuplicateKey {
                    table: "bcast_urgent_followup_policy_ledger",
                    key: row.bcast_urgent_followup_policy_event_id.to_string(),
                });
            }
        }

        let mut ordered = self.bcast_urgent_followup_policy_ledger.clone();
        ordered.sort_by_key(|row| row.bcast_urgent_followup_policy_event_id);
        let mut tenant_policy_versions: BTreeMap<TenantId, u64> = BTreeMap::new();
        for row in ordered {
            validate_comms_idempotency_key(
                "bcast_urgent_followup_policy_ledger.idempotency_key",
            ...
            if row.policy_version != expected_policy_version {
                return Err(StorageError::ContractViolation(
                    ContractViolation::InvalidValue {
                        field: "bcast_urgent_followup_policy_ledger.policy_version",
                        reason: "must increment by 1 per tenant",
                    },
                ));
            }
            tenant_policy_versions.insert(row.tenant_id.clone(), row.policy_version);
            self.bcast_urgent_followup_policy_idempotency_index.insert(
                (row.tenant_id.clone(), row.idempotency_key.clone()),
                row.bcast_urgent_followup_policy_event_id,
            );
            self.apply_bcast_urgent_followup_policy_event_to_current(&row);
        }
        Ok(())
    ...
    pub fn attempt_overwrite_bcast_urgent_followup_policy_event(
        &mut self,
        _bcast_urgent_followup_policy_event_id: u64,
    ) -> Result<(), StorageError> {
        Err(StorageError::AppendOnlyViolation {
            table: "bcast_urgent_followup_policy_ledger",
        })
    ...
    fn ph1rem_schedule(
        ...
        let occurrence_times =
            reminder_occurrence_times(resolved_due_at, r.recurrence_rule.as_deref());
        ...
        for scheduled_time in occurrence_times {
            ...
            let occurrence_id = ReminderOccurrenceId::new(format!(
                "occ_{:016x}",
                self.next_reminder_occurrence_seq
            ))
            .map_err(StorageError::ContractViolation)?;
    ...
    fn ph1rem_update(
        ...
        if parsed_due.is_some() || recurrence_changed {
            ...
            for scheduled_time in occurrence_times {
                self.next_reminder_occurrence_seq =
                    self.next_reminder_occurrence_seq.saturating_add(1);
                let occurrence_id = ReminderOccurrenceId::new(format!(
                    "occ_{:016x}",
                    self.next_reminder_occurrence_seq
                ))
    ...
    fn ph1rem_deliver_due(
        ...
        let mut status = if r.offline_state {
            ReminderDeliveryStatus::RetryScheduled
        } else {
            ReminderDeliveryStatus::Delivered
        };
        let mut state_after: Option<ReminderState> = None;
        let mut retry_time: Option<MonotonicTimeNs> = None;
        let mut followup_time: Option<MonotonicTimeNs> = None;

        if req.envelope.now.0 < occ_snapshot.scheduled_time.0 {
            status = ReminderDeliveryStatus::RetryScheduled;
            retry_time = Some(occ_snapshot.scheduled_time);
        }
        if let Some(snooze_until) = occ_snapshot.snooze_until {
            if req.envelope.now.0 < snooze_until.0 {
                status = ReminderDeliveryStatus::RetryScheduled;
                state_after = Some(ReminderState::Snoozed);
                retry_time = Some(snooze_until);
            }
        }

        if status == ReminderDeliveryStatus::RetryScheduled {
            if retry_time.is_none() {
                retry_time = Some(MonotonicTimeNs(
                    req.envelope
                        .now
                        .0
                        .saturating_add(REMINDER_DELIVERY_RETRY_DELAY_NS),
                ));
            }
        } else {
            // Due delivery always schedules a bounded follow-up checkpoint.
            state_after = Some(ReminderState::FollowupPending);
            let followup_delay_ns = if req.envelope.now.0
                > occ_snapshot
                    .scheduled_time
                    .0
                    .saturating_add(REMINDER_DELIVERY_MISSED_GRACE_NS)
            {
                // Missed reminder path: still fire once, then tighten follow-up timing.
                REMINDER_DELIVERY_RETRY_DELAY_NS
            } else {
                REMINDER_DELIVERY_FOLLOWUP_DELAY_NS
            };
            followup_time = Some(MonotonicTimeNs(
                req.envelope.now.0.saturating_add(followup_delay_ns),
            ));
        }

        let reason_code = match status {
            ReminderDeliveryStatus::Delivered => reminder_reason_codes::REMINDER_DELIVERED_DUE,
            ReminderDeliveryStatus::RetryScheduled => {
                reminder_reason_codes::REMINDER_RETRY_SCHEDULED
            }
            ReminderDeliveryStatus::DeferredQuietHours => {
                reminder_reason_codes::REMINDER_RETRY_SCHEDULED
            }
            ReminderDeliveryStatus::Failed => reminder_reason_codes::REMINDER_FAILED,
        };
        self.ph1rem_delivery_common(
            req,
            &r.tenant_id,
    ...
    fn ph1rem_escalate(
        ...
        let reminder_row = self.reminders.get(&r.reminder_id).ok_or_else(|| {
            StorageError::ContractViolation(ContractViolation::InvalidValue {
                field: "ph1rem.delivery.reminder_id",
                reason: "not found",
            })
        })?;
        let to_channel_allowed = reminder_row
            .channel_preferences
            .iter()
            .any(|channel| *channel == r.to_channel);
        let prior_attempt_exists = self.reminder_delivery_attempts.iter().any(|attempt| {
            attempt.reminder_id == r.reminder_id
                && attempt.occurrence_id == r.occurrence_id
                && attempt.channel == r.from_channel
        });
        if !to_channel_allowed || !prior_attempt_exists {
            return Ok(Ph1RemResponse::Refuse(Ph1RemRefuse::v1(
                req.envelope.simulation_id.clone(),
                reminder_reason_codes::REM_FAIL_POLICY_BLOCKED,
                "escalation requires policy-approved target channel and prior from_channel attempt"
                    .to_string(),
            )?));
        }
        self.ph1rem_delivery_common(
            ...
            Some(1),
            Some(ReminderState::FollowupPending),
            None,
            Some(MonotonicTimeNs(
                req.envelope
                    .now
                    .0
                    .saturating_add(REMINDER_DELIVERY_FOLLOWUP_DELAY_NS),
            )),
            false,
        )
    }
    ...
    fn ph1rem_delivery_common(
        ...
        state_after: Option<ReminderState>,
        retry_time: Option<MonotonicTimeNs>,
        followup_time: Option<MonotonicTimeNs>,
        dedupe_delivered_on_occurrence_channel: bool,
    ) -> Result<Ph1RemResponse, StorageError> {
        let (row_state, occ_scheduled_time) = {
            ...
            if occ.state == ReminderState::Canceled
                || occ.state == ReminderState::Completed
                || occ.state == ReminderState::Failed
            {
                return Ok(Ph1RemResponse::Refuse(Ph1RemRefuse::v1(
                    req.envelope.simulation_id.clone(),
                    reminder_reason_codes::REM_FAIL_STATE_TRANSITION_INVALID,
                    "cannot deliver terminal occurrence state".to_string(),
                )?));
            }
            (row.state, occ.scheduled_time)
        };

        if dedupe_delivered_on_occurrence_channel
            && delivery_status == ReminderDeliveryStatus::Delivered
            && {
                self.reminder_delivery_attempts.iter().any(|entry| {
            ...
            }
        {
            let existing = self
                ...
                .expect("delivery attempt must exist after any()");
            let out_state = self
                .reminders
                .get(reminder_id)
                .map(|row| row.state)
                .unwrap_or(row_state);
            let out_scheduled_time = self
                .reminder_occurrences
                .get(occurrence_id)
                .map(|occ| occ.scheduled_time)
                .unwrap_or(occ_scheduled_time);
            return Ok(Ph1RemResponse::Ok(Ph1RemOk::v1(
                req.envelope.simulation_id.clone(),
                reason_code,
                reminder_id.clone(),
            ...
            .get(delivery_attempt_id)
        {
            ...
            if existing.reminder_id != *reminder_id || existing.occurrence_id != *occurrence_id {
                return Ok(Ph1RemResponse::Refuse(Ph1RemRefuse::v1(
                    req.envelope.simulation_id.clone(),
                    reminder_reason_codes::REM_FAIL_SCOPE_VIOLATION,
                    "delivery_attempt_id already bound to another reminder occurrence".to_string(),
                )?));
            }
            let out_state = self
                .reminders
        ...
        if state_after.is_some() || retry_time.is_some() || followup_time.is_some() {
            let mut row = self.reminders.remove(reminder_id).ok_or_else(|| {
                StorageError::ContractViolation(ContractViolation::InvalidValue {
                    field: "ph1rem.delivery.reminder_id",
                    reason: "not found",
            ...
            if let Some(new_state) = state_after {
                row.state = new_state;
                occ.state = new_state;
            }
            if let Some(new_retry_time) = retry_time {
                occ.retry_time = Some(new_retry_time);
            }
            if let Some(new_followup_time) = followup_time {
                occ.followup_time = Some(new_followup_time);
            }
            occ.updated_at = req.envelope.now;
            row.updated_at = req.envelope.now;
            self.reminders.insert(reminder_id.clone(), row);
...
mod tests {
    ...
    #[test]
    fn at_f_15_bcast_urgent_followup_policy_append_only_idempotent_and_replayable() {
        let mut s = Ph1fStore::new_in_memory();
        let tenant_id = tenant();
        let actor = sender();
        let event_id = s
            .append_bcast_urgent_followup_policy_update_event(
                tenant_id.clone(),
                false,
                actor.clone(),
                CorrelationId(9303),
                "bcast_urgent_followup_policy_update_1".to_string(),
                MonotonicTimeNs(33),
        ...
        let event_id_retry = s
            ...
            .unwrap();
        assert_eq!(event_id, event_id_retry);
        assert_eq!(s.bcast_urgent_followup_policy_ledger().len(), 1);
        assert!(!s.bcast_urgent_followup_immediate_for_tenant(&tenant_id));
        let current_before = s.bcast_urgent_followup_policy_current().clone();
        s.rebuild_bcast_urgent_followup_policy_current_from_ledger()
            .unwrap();
        assert_eq!(
            current_before,
            s.bcast_urgent_followup_policy_current().clone()
        );
        assert!(matches!(
            s.attempt_overwrite_bcast_urgent_followup_policy_event(event_id),
            Err(StorageError::AppendOnlyViolation { .. })
        ));
    }

    #[test]
    fn at_f_16_bcast_urgent_followup_policy_rebuild_rejects_invalid_policy_version() {
        let mut s = Ph1fStore::new_in_memory();
        let tenant_id = tenant();
        s.bcast_urgent_followup_policy_ledger
            .push(BcastUrgentFollowupPolicyLedgerRow {
                schema_version: PH1BCAST_CONTRACT_VERSION,
                bcast_urgent_followup_policy_event_id: 1,
                tenant_id: tenant_id.clone(),
                urgent_followup_immediate: true,
                updated_by_user_id: sender(),
                correlation_id: CorrelationId(9304),
                idempotency_key: "bcast_urgent_followup_policy_update_seed_1".to_string(),
                created_at: MonotonicTimeNs(34),
                reason_code: ReasonCodeId(0x4243_00A7),
                policy_version: 1,
            });
        s.bcast_urgent_followup_policy_ledger
            .push(BcastUrgentFollowupPolicyLedgerRow {
                schema_version: PH1BCAST_CONTRACT_VERSION,
                bcast_urgent_followup_policy_event_id: 2,
                tenant_id: tenant_id.clone(),
                urgent_followup_immediate: false,
                updated_by_user_id: sender(),
                correlation_id: CorrelationId(9305),
                idempotency_key: "bcast_urgent_followup_policy_update_seed_2".to_string(),
                created_at: MonotonicTimeNs(35),
                reason_code: ReasonCodeId(0x4243_00A8),
                policy_version: 3,
            });

        let rebuild = s.rebuild_bcast_urgent_followup_policy_current_from_ledger();
        assert!(matches!(
            rebuild,
            Err(StorageError::ContractViolation(
                ContractViolation::InvalidValue {
    ...
    #[test]
    fn at_f_17_bcast_policy_registry_append_only_idempotent_and_replayable() {
        let mut s = Ph1fStore::new_in_memory();
        let tenant_id = tenant();
        let actor = sender();
        let event_id = s
            .append_bcast_policy_update_event(
                ...
                actor.clone(),
                CorrelationId(9306),
                "bcast_policy_update_1".to_string(),
                MonotonicTimeNs(36),
        ...
        let event_id_retry = s
            ...
            .unwrap();
        assert_eq!(event_id, event_id_retry);
        assert_eq!(s.bcast_policy_ledger().len(), 1);
        assert_eq!(s.bcast_non_urgent_wait_seconds_for_tenant(&tenant_id), 420);

        let current_before = s.bcast_policy_current().clone();
        s.rebuild_bcast_policy_current_from_ledger().unwrap();
        assert_eq!(current_before, s.bcast_policy_current().clone());

        assert!(matches!(
            s.attempt_overwrite_bcast_policy_event(event_id),
            Err(StorageError::AppendOnlyViolation { .. })
        ));
    }

    #[test]
    fn at_f_18_bcast_policy_registry_report_rows_include_old_new_and_metadata() {
        let mut s = Ph1fStore::new_in_memory();
        let tenant_id = tenant();
        let actor = sender();

        s.append_bcast_policy_update_event(
            tenant_id.clone(),
            BcastPolicyUpdateValue::NonUrgentWaitSeconds(420),
            actor.clone(),
            CorrelationId(9307),
            "bcast_policy_update_2".to_string(),
            MonotonicTimeNs(37),
        ...
        s.append_bcast_policy_update_event(
            ...
            actor.clone(),
            CorrelationId(9308),
            "bcast_policy_update_3".to_string(),
            MonotonicTimeNs(38),
            ReasonCodeId(0x4243_00B3),
        )
        .unwrap();

        let report = s.bcast_policy_changes_for_tenant(&tenant_id, 10);
        assert_eq!(report.len(), 2);
        assert_eq!(
            report[0].setting_key,
            BcastPolicySettingKey::UrgentFollowupMode
        );
        assert_eq!(report[0].old_value, "immediate");
        assert_eq!(report[0].new_value, "wait");
        assert_eq!(report[0].reason_code, ReasonCodeId(0x4243_00B3));
        assert_eq!(report[0].idempotency_key, "bcast_policy_update_3");
        assert_eq!(
            report[1].setting_key,
            BcastPolicySettingKey::NonUrgentWaitSeconds
        );
        assert_eq!(report[1].old_value, "300");
        assert_eq!(report[1].new_value, "420");
    }
