...
- Phone-first lock: onboarding start requires app context (`app_platform`, `app_instance_id`, `deep_link_nonce`, `link_opened_at`).
...
- ONB start requires `LINK_OPEN_ACTIVATE` app-open context (`app_platform`, `app_instance_id`, `deep_link_nonce`, `link_opened_at`).
...
|---|---|---|---|---|---|---:|---:|---:|---|
| ONB_INVITED_S01 | PH1.ONB | PH1ONB_SESSION_START_DRAFT_ROW | token_id, device_fingerprint, app_platform, app_instance_id, deep_link_nonce, link_opened_at | onboarding_session_id, onboarding_state, pinned_schema_context, required_verification_gates | DB_WRITE (simulation-gated) | 700 | 2 | 250 | [ONB_START_RETRYABLE] |
...
| ONB_INVITED_S09 | PH1.VOICE.ID | VID_ENROLL_START_DRAFT_ROW | onboarding_session_id, device_id, consent_asserted, max_total_attempts, max_session_enroll_time_ms, lock_after_consecutive_passes | voice_enrollment_session_id | DB_WRITE (simulation-gated) | 700 | 2 | 250 | [VID_IDEMPOTENCY_REPLAY] |
| ONB_INVITED_S10 | PH1.VOICE.ID | VID_ENROLL_SAMPLE_COMMIT_ROW | voice_enrollment_session_id, audio_sample_ref, attempt_index, sample_result, reason_code?, idempotency_key | enrollment_progress | DB_WRITE (simulation-gated) | 900 | 3 | 250 | [VID_IDEMPOTENCY_REPLAY] |
| ONB_INVITED_S11 | PH1.VOICE.ID | VID_ENROLL_COMPLETE_COMMIT_ROW | voice_enrollment_session_id, idempotency_key | voice_profile_id, voice_artifact_sync_receipt_ref, enrollment_complete | DB_WRITE (simulation-gated) | 700 | 2 | 250 | [VID_IDEMPOTENCY_REPLAY] |
| ONB_INVITED_S12 | PH1.W | PH1W_ENROLL_START_DRAFT_ROW | onboarding_session_id, user_id, device_id, pass_target, max_attempts, enrollment_timeout_ms, idempotency_key | wake_enrollment_session_id | DB_WRITE (simulation-gated, conditional: run only when wake-required platform policy is active; default `ANDROID` yes, `IOS` no) | 700 | 2 | 250 | [W_IDEMPOTENCY_REPLAY] |
| ONB_INVITED_S13 | PH1.W | PH1W_ENROLL_SAMPLE_COMMIT_ROW | wake_enrollment_session_id, sample_duration_ms, vad_coverage, snr_db, clipping_pct, rms_dbfs, noise_floor_dbfs, peak_dbfs, overlap_ratio, result, reason_code?, idempotency_key | wake_enrollment_progress | DB_WRITE (simulation-gated, conditional: run only when wake-required platform policy is active) | 900 | 3 | 250 | [W_IDEMPOTENCY_REPLAY] |
| ONB_INVITED_S14 | PH1.W | PH1W_ENROLL_COMPLETE_COMMIT_ROW | wake_enrollment_session_id, wake_profile_id, idempotency_key | wake_enrollment_complete, wake_artifact_sync_receipt_ref | DB_WRITE (simulation-gated, conditional: run only when wake-required platform policy is active) | 700 | 2 | 250 | [W_IDEMPOTENCY_REPLAY] |
...
| ONB_INVITED_S16 | PH1.ONB | PH1ONB_COMPLETE_COMMIT_ROW | onboarding_session_id, idempotency_key, voice_artifact_sync_receipt_ref, wake_artifact_sync_receipt_ref? | onboarding_complete | DB_WRITE (simulation-gated; wake receipt required only when wake-required platform policy is active) | 700 | 2 | 250 | [ONB_COMPLETE_RETRYABLE] |
...
- `AT-PBS-ONB-03`: Voice enrollment is always simulation-gated; wake enrollment is simulation-gated only on wake-required platform policy paths.
...
- `AT-PBS-ONB-11`: ONB start fails closed when app-open context is missing/invalid.
- `AT-PBS-ONB-12`: ONB completion requires voice sync receipt refs when locked voice enrollment exists; wake sync receipt refs are required only on wake-required platform policy paths.
- `AT-PBS-ONB-13`: Wake enrollment start tied to an `IOS` onboarding session is refused by default (`explicit_trigger_only=true`) unless policy override is explicitly enabled.
