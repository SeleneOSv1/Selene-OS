# TOOL_WEB_SEARCH Blueprint Record

## 1) Blueprint Header
- `process_id`: `TOOL_WEB_SEARCH`
- `intent_type`: `TOOL_WEB_SEARCH`
- `version`: `v1`
- `status`: `ACTIVE`
...
|---|---|---|---|---|---|---:|---:|---:|---|
| WEB_SEARCH_S01 | PH1.E | PH1E_TOOL_WEB_SEARCH_QUERY | tenant_id, correlation_id, turn_id, user_id, device_id, tool_name=WEB_SEARCH, query_hash, cache_status, reason_code | tool_response | READ_ONLY | 250 | 1 | 100 | [E_FAIL_TIMEOUT] |
| WEB_SEARCH_S02 | PH1.X | PH1X_TOOL_RESPONSE_RENDER | tool_response | response_text, source_list, retrieved_at | READ_ONLY | 250 | 1 | 100 | [OS_RESPONSE_RETRYABLE] |
...
## 8) Acceptance Tests
- `AT-PBS-TOOLWEB-01`: Web search routes to PH1.E tool dispatch (not simulation dispatch).
- `AT-PBS-TOOLWEB-02`: Response includes provenance (`source`, `retrieved_at`) and remains read-only.
