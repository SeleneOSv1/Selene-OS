...
|---|---|---|---|---|---|---:|---:|---:|---|
| TIME_S01 | PH1.E | PH1E_TOOL_OK_COMMIT_ROW | tenant_id, correlation_id, turn_id, user_id, device_id, tool_name=TIME, query_hash, cache_status, reason_code, idempotency_key | audit_event_id | DB_WRITE | 250 | 1 | 100 | [E_FAIL_TIMEOUT] |
| TIME_S02 | PH1.X | PH1X_RESPOND_COMMIT_ROW | time_result | response_text | DB_WRITE | 250 | 1 | 100 | [OS_RESPONSE_RETRYABLE] |

Notes:
- The actual time lookup is performed by the read-only tool router; PH1.E records tool outcome only.
