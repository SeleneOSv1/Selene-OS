...
| memory tombstone / delete accepted | PH1.M authoritative ledger/config commit first (`Forgotten` atom-ledger event, `THREAD_FORGOTTEN` thread-ledger event, or additive PH1.M-owned marker for explicit delete-request acceptance where needed) | PH1.J audit required; canonical proof linkage required only for protected or governed memory paths | conditional when governed retention/hold policy applies | conditional when live protected execution admissibility changes | complete when PH1.M authoritative commit exists and any required visibility is recorded; derived cleanup may lag without reopening the decision | replay re-reads the PH1.M authoritative subject and only repairs derived, audit, or receipt lag | fail closed on missing hold/policy context; no current/index/evidence surface may substitute |
