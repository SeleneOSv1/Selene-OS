# H96 APP_IPHONE First Canonical `INTERRUPT_VISIBLE` Cloud-Authored Clarify Sensitivity-Level Visible Native Shell Implementation Build Plan

- This is the first canonical APP_IPHONE `INTERRUPT_VISIBLE` cloud-authored clarify sensitivity-level visible native shell implementation build.
...
- H88 route seeding remains live through exact `session_state=suspended`, `session_id`, `next_allowed_actions_may_speak`, `next_allowed_actions_must_wait`, `next_allowed_actions_must_rewake`, `recovery_mode`, and `reconciliation_decision`, and this H96 run preserves that contract.
- H87 / H86 / H85 route seeding remains live and this H96 run preserves those contracts.
...
- cloud-authored sensitivity evidence remains rendered only and never upgrades local authority or access posture.
...
- this run adds bounded clarify sensitivity-level visibility only.
- this run preserves H79-H95 surfaces, exact H83 / H84 / H85 / H86 / H87 / H88 / H89 / H90 / H91 / H92 / H93 / H94 / H95 copy, H74-H77 takeover fields, exact setup-receipt visibility, and H83 / H84 producer copy.
- this run does not add local interrupt authoring, local ambiguity inference, local field inference, local routing guidance, local confirmation law, local sensitivity policy, local authority upgrade, local onboarding authority, local gate bypass, subject-relation-confidence rendering, heavy resume-buffer detail hydration, wake parity, side-button producer wiring, or autonomous unlock.
- APP_IPHONE remains first-class but non-authority, `EXPLICIT_ONLY`, and cloud-authoritative parity only.
- native iPhone shell now exposes bounded `INTERRUPT_VISIBLE` clarify sensitivity-level visibility in lawful `EXPLICIT_ONLY` cloud-authored posture.
...
- no post-H96 next exact winner is published in this run.
