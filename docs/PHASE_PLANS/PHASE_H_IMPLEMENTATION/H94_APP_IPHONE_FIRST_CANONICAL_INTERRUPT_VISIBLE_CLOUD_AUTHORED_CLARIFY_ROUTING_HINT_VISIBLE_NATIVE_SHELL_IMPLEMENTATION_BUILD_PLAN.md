# H94 APP_IPHONE First Canonical `INTERRUPT_VISIBLE` Cloud-Authored Clarify Routing-Hint Visible Native Shell Implementation Build Plan

- This is the first canonical APP_IPHONE `INTERRUPT_VISIBLE` cloud-authored clarify routing-hint visible native shell implementation build.
...
- H88 route seeding remains live through exact `session_state=suspended`, `session_id`, `next_allowed_actions_may_speak`, `next_allowed_actions_must_wait`, `next_allowed_actions_must_rewake`, `recovery_mode`, and `reconciliation_decision`, and this H94 run preserves that contract.
- H87 / H86 / H85 route seeding remains live and this H94 run preserves those contracts.
...
- this run adds bounded clarify routing-hint visibility only.
- this run preserves H79-H93 surfaces, exact H83 / H84 / H85 / H86 / H87 / H88 / H89 / H90 / H91 / H92 / H93 copy, H74-H77 takeover fields, exact setup-receipt visibility, and H83 / H84 producer copy.
- this run does not add local interrupt authoring, local ambiguity inference, local field inference, local routing guidance, local onboarding authority, local gate bypass, subject-relation-confidence rendering, heavy resume-buffer detail hydration, wake parity, side-button producer wiring, or autonomous unlock.
- APP_IPHONE remains first-class but non-authority, `EXPLICIT_ONLY`, and cloud-authoritative parity only.
- native iPhone shell now exposes bounded `INTERRUPT_VISIBLE` clarify routing-hint visibility in lawful `EXPLICIT_ONLY` cloud-authored posture.
...
- no post-H94 next exact winner is published in this run.
