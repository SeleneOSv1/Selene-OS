# H84 APP_IPHONE: First Canonical `EXPLICIT_ENTRY_READY` Explicit Voice-Turn Request Production Native Shell Implementation Build

## Objective

This is the first canonical APP_IPHONE `EXPLICIT_ENTRY_READY` explicit voice-turn request production native shell implementation build.
...
H79 implementation remains live as the first canonical recent thread window / typed input affordance / explicit voice entry affordance native shell implementation build, and this H84 run advances only the explicit voice affordance while preserving the recent thread window and typed-turn production surfaces.
...
G1 already freezes `EXPLICIT_ENTRY_READY` as the dominant session surface in explicit-entry posture with typed input and lawful explicit voice entry.
...
This run adds bounded explicit voice-turn request production in lawful `EXPLICIT_ONLY` cloud-authoritative posture.

This run adds lawful iPhone permission keys for explicit microphone and speech-recognition use only.
...
This run does not add wake-word behavior, side-button producer wiring, wake parity, or autonomous unlock.
...
no post-H84 next exact winner is published in this run.

## Out Of Scope

This H84 implementation does not authorize:
