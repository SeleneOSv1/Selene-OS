...
The H363 purpose is to make low-risk public deterministic time clarification behave like one continuous same-thread request: incomplete or ambiguous time questions ask one clean clarification, and the immediate lawful follow-up completes the same pending time request without Voice ID, protected-action widening, fake local answer behavior, PH1.D redesign, provider redesign, shell redesign, or backend route expansion.
...
- `what time is it in Spain` returns a clean clarification instead of guessing across multiple timezones.
- `Madrid` immediately after the Spain clarification returns a clean final answer for Madrid in the same thread.
- `what time is it in Portugal` returns a lawful clarification when required.
- `Lisbon` immediately after the Portugal clarification returns a clean final answer for Lisbon in the same thread.
...
- Make ambiguous place deterministic time queries ask one clean clarification instead of guessing.
...
- Complete follow-ups such as `Madrid` after Spain and `Lisbon` after Portugal through the same deterministic time lane.
...
- `Madrid` immediately after the Spain clarification completes the same pending deterministic time request and returns a clean Madrid answer in the same thread.
- `what time is it in Portugal` produces a lawful clarification if required.
- `Lisbon` immediately after the Portugal clarification completes the same pending deterministic time request and returns a clean Lisbon answer in the same thread.
...
- Ambiguous time queries clarify instead of guessing.
