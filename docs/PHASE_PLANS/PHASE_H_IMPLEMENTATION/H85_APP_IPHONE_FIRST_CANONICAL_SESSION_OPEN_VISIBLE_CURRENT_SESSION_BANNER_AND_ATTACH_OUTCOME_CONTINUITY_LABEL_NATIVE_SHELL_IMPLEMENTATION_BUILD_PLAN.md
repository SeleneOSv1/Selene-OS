# H85 APP_IPHONE: First Canonical `SESSION_OPEN_VISIBLE` Current Session Banner And Attach-Outcome Continuity Label Native Shell Implementation Build

## Objective

This is the first canonical APP_IPHONE `SESSION_OPEN_VISIBLE` current session banner and attach-outcome continuity label native shell implementation build.
...
H79 implementation remains live as the first canonical recent thread window / typed input affordance / explicit voice entry affordance native shell implementation build, and this H85 run advances only `SESSION_OPEN_VISIBLE` while preserving the H79-H84 surfaces.
...
G1 already freezes `SESSION_OPEN_VISIBLE` as the dominant session surface visible, ready for the next explicit turn, with a current session banner and attach-outcome continuity labeling only.
...
Current shared source still preserves `OsVoiceLiveTurnInput`.

Current shared source still preserves `OsVoiceTurnContext`.

Current shared source still preserves `OsVoiceTrigger::Explicit`.

Current shared source still preserves `Ph1VoiceIdRequest`.

Current shared source still preserves `VoiceIdentityEmbeddingGateProfiles::mvp_v1_phone_first()`.

Current shared source still preserves `ios_explicit`.
...
Current shared source still preserves `SessionAttachOutcome`.

Current shared source still preserves `session_attach_outcome` inside `RuntimeExecutionEnvelope`.
...
This run adds bounded `SESSION_OPEN_VISIBLE` current session banner and attach-outcome continuity label in lawful `EXPLICIT_ONLY` cloud-authoritative posture.
...
This run preserves the H83 typed-turn request production surface and exact typed / voice request-production copy.
...
no post-H85 next exact winner is published in this run.

## Out Of Scope

This H85 implementation does not authorize:

- `SESSION_ACTIVE_VISIBLE`
