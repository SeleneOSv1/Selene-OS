# H79 APP_IPHONE: First Canonical `EXPLICIT_ENTRY_READY` Recent Thread Window / Typed Input Affordance / Explicit Voice Entry Affordance Native Shell Implementation Build

## Objective

This is the first canonical APP_IPHONE `EXPLICIT_ENTRY_READY` recent thread window / typed input affordance / explicit voice entry affordance native shell implementation build.
...
G1 already freezes `EXPLICIT_ENTRY_READY` as the dominant session surface in explicit-entry posture with recent thread window, typed input affordance, and lawful explicit voice entry affordance.
...
The pre-H79 native iPhone shell already preserved `onOpenURL`, `EXPLICIT_ENTRY_READY`, `ONBOARDING_ENTRY_ACTIVE`, exact setup-receipt display, and the H74 / H75 / H76 / H77 takeover fields, but still rendered `EXPLICIT_ENTRY_READY` as a waiting card only with placeholder `Session`, `History`, and `System Activity` surfaces.
...
This run adds a bounded native `EXPLICIT_ENTRY_READY` dominant session surface that exposes a recent thread window, typed input affordance, and explicit voice entry affordance in read-only non-producing posture.
...
This run does not add networking, persistence, runtime request production, local authority, invite activation, onboarding mutation, typed-turn dispatch, explicit voice-turn dispatch, history-side-drawer behavior, `System Activity` queue behavior, `Needs Attention` queue behavior, voice-artifact sync behavior, access-engine activation, side-button producer wiring, wake parity, or autonomous unlock.

No post-H79 next exact winner is published in this run.

## Out Of Scope

This H79 implementation does not authorize:

- APP_IPHONE typed-turn dispatch
- APP_IPHONE explicit voice-turn dispatch
- history-side-drawer behavior
- `System Activity` queue behavior
- `Needs Attention` queue behavior
- voice-artifact sync behavior
