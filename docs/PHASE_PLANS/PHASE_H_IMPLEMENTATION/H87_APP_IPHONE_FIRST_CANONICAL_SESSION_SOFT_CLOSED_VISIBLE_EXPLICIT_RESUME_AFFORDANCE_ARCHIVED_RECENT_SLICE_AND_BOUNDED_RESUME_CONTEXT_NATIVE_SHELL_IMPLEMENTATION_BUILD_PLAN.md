# H87 APP_IPHONE: First Canonical `SESSION_SOFT_CLOSED_VISIBLE` Explicit Resume Affordance / Archived Recent Slice / Bounded Resume Context Native Shell Implementation Build

## Objective

This is the first canonical APP_IPHONE `SESSION_SOFT_CLOSED_VISIBLE` explicit resume affordance / archived recent slice / bounded PH1.M `resume context` native shell implementation build.
...
- H86 route seeding remains live through exact `session_state`, `session_id`, `turn_id`, `current_user_turn_text`, `current_selene_turn_text`, and `current_governed_output_summary` handling for bounded `SESSION_ACTIVE_VISIBLE` preview, and this H87 run preserves that contract while adding bounded soft-closed preview inputs.
...
- H85 route seeding remains live through exact `session_state`, `session_id`, and `session_attach_outcome` handling for bounded `SESSION_OPEN_VISIBLE` preview, and this H87 run preserves that contract.
...
- H79 implementation remains live as the first canonical recent thread window / typed input affordance / explicit voice entry affordance native shell implementation build, and this H87 run advances only `SESSION_SOFT_CLOSED_VISIBLE` while preserving the H79-H86 surfaces.
...
- current shared source still preserves PH1.M resume-select carriers `selected_thread_id`, `selected_thread_title`, `pending_work_order_id`, `resume_tier`, and `resume_summary_bullets`.
...
- this run adds bounded `SESSION_SOFT_CLOSED_VISIBLE` explicit resume affordance, archived recent slice, and bounded PH1.M `resume context` in lawful `EXPLICIT_ONLY` cloud-authoritative posture.
...
- this run preserves the H79-H86 surfaces and exact typed / voice / open / active request-visibility copy.
- this run preserves H85 open-session route seeding through exact `session_state`, `session_id`, and `session_attach_outcome` handling and preserves H86 active-session route seeding through exact `session_state`, `session_id`, `turn_id`, `current_user_turn_text`, `current_selene_turn_text`, and `current_governed_output_summary` handling while adding bounded soft-closed preview seeding through `selected_thread_id`, `selected_thread_title`, `pending_work_order_id`, `resume_tier`, `resume_summary_bullets`, `archived_user_turn_text`, and `archived_selene_turn_text`.
- native iPhone shell now exposes bounded `SESSION_SOFT_CLOSED_VISIBLE` explicit resume affordance, archived recent slice, and bounded PH1.M `resume context` in lawful `EXPLICIT_ONLY` cloud-authoritative posture.
- this run does not add `SESSION_SUSPENDED_VISIBLE`, `RECOVERING`, `DEGRADED_RECOVERY`, `QUARANTINED_LOCAL_STATE`, `INTERRUPT_VISIBLE`, explicit active-session interrupt controls, heavy governed-content loaders, wake-word behavior, side-button producer wiring, wake parity, or autonomous unlock.
...
- no autonomous unlock claim is lawful in this run.
- APP_MAC_DESKTOP remains later in dependency order and is not selected in this run.
- no post-H87 next exact winner is published in this run.
