# H99 APP_IPHONE First Canonical `INTERRUPT_VISIBLE` Cloud-Authored Return-Check-Expiry Visible Native Shell Implementation Build Plan

- This is the first canonical APP_IPHONE `INTERRUPT_VISIBLE` cloud-authored return-check-expiry visible native shell implementation build.
...
- H88 route seeding remains live through exact `session_state=suspended`, `session_id`, `next_allowed_actions_may_speak`, `next_allowed_actions_must_wait`, `next_allowed_actions_must_rewake`, `recovery_mode`, and `reconciliation_decision`, and this H99 run preserves that contract.
- H87 / H86 / H85 route seeding remains live and this H99 run preserves those contracts.
- current shared source already preserves `InterruptSubjectRelation::{Same, Switch, Uncertain}`, `interrupt_subject_relation_confidence`, `InterruptContinuityOutcome::{SameSubjectAppend, SwitchTopicThenReturnCheck}`, `InterruptResumePolicy::{ResumeNow, ResumeLater, Discard}`, `ConfirmAnswer::{Yes, No}`, `ClarifyDirective`, PH1.N `requires_confirmation`, PH1.N `ambiguity_flags`, PH1.N `routing_hints`, `SensitivityLevel::{Public, Private, Confidential}`, and PH1.X continuity state including `return_check_pending` and `return_check_expires_at`.
- cloud-authored return-check-expiry evidence remains evidence-only downstream posture and never upgrades local timer law or dispatch posture.
...
- this run adds bounded return-check-expiry visibility only.
- this run preserves H79-H98 surfaces, exact H83 / H84 / H85 / H86 / H87 / H88 / H89 / H90 / H91 / H92 / H93 / H94 / H95 / H96 / H97 / H98 copy, H74-H77 takeover fields, exact setup-receipt visibility, and H83 / H84 producer copy.
- this run does not add local interrupt authoring, local ambiguity inference, local field inference, local routing guidance, local confirmation law, local sensitivity policy, local threshold law, local timer law, local subject binding, local identity resolution, local subject reconciliation authority, local onboarding authority, local gate bypass, live countdown rendering, dedicated resume-buffer-live rendering, dedicated resume-buffer-topic-hint rendering, heavy resume-buffer detail hydration, `tts_resume_snapshot`, wake parity, side-button producer wiring, or autonomous unlock.
- APP_IPHONE remains first-class but non-authority, `EXPLICIT_ONLY`, and cloud-authoritative parity only.
- native iPhone shell now exposes bounded `INTERRUPT_VISIBLE` return-check-expiry visibility in lawful `EXPLICIT_ONLY` cloud-authored posture.
...
- no post-H99 next exact winner is published in this run.
